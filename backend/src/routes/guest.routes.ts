...
router.post("/signup", async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const check = guestRegisterCredentials({ name, email, password, gender, dob, bio });
        ...
        const newGuest = await prisma.guest.create({ data: { name, email, password: hashed_password, gender, dob, bio } })
